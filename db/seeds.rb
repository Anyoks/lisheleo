...
# 		# puts "Day:: #{day}"
# 		# puts "Start time & end time:: #{start_time} #{end_time}"
# 		# # puts "found--- #{start_time.min}"

# 		puts "\n"
# 		if program.available_times.where( day: day ,start_time: start_time, end_time: end_time).present?
# 			puts "#{program.name} ->#{day} :: #{start_time} #{end_time} ALREADY PRESENT"
# 		else
			
# 			program.available_times.where( day: day ,start_time: start_time, end_time: end_time).first_or_create do |time|
...
# 				# puts "Time #{start_time}  end #{end_time}"
# 				puts "#{program.name} -> #{day} :: #{start_time} #{end_time} CREATED"
...
# names = ['personalized wellness','therapeutic massage','keep fit','cancer support group']
# codes = ['B1','B2','B3','B4']
...
# 	pg = Program.where(name: names[i]).first

# 	puts "found #{pg.name}...... adding code #{codes[i]}"
# 	pg.update_attributes(code: codes[i])
# 	i += 1
# end

# ['create', 'update', 'delete'].each do |type|
# 	ActivityType.find_or_create_by({name: type})
# end

['male', 'female'].each do |type|
	Gender.find_or_create_by({gender: type})
end

['married', 'single', 'divorced', 'widowed'].each do |type|
	Marital.find_or_create_by({status: type})
end